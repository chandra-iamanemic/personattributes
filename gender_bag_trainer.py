...
# In this case we extract only the Gender and Backpack attribute for the images and form a dataset
for i in tqdm(range(len(data['market_attribute'][0][0][1][0][0][0][0]))):
    
    # Extract the Person ID from the .mat matrix    
    ...
    # Extract the backpack attribute (I have named it bag but its the backpack attribute)
    bag = data['market_attribute'][0][0][1][0][0][1][0][i]

    
    gender_one_hot = np.eye(2)[gender-1].reshape(1,2)
    bag_one_hot = np.eye(2)[bag-1].reshape(1,2)
    combined_one_hot = np.array([gender_one_hot, bag_one_hot]).reshape(1,4)
...
# Since we are training for two attributes which I have vectorized as a (1,4) vector 
# The first two elements of this vector are the one hot vector of the gender attribute
# The second two elements of this vector are the one hot vector of the backpack attribute
# Hence the final layer must have 4 nodes to train on this output label of 4 elements
...
torch.save(model.state_dict(), "C:/Users/Chandrasekar/Documents/signatrix test/Market-1501_Attribute-master/weights/z.pt")
...
# Run through the test set and evaluate the accuracy for each of the Attribute (Gender and Bag)
for i in tqdm(range(len(test_x))):
    
    pred = model(test_x[i:i+1])
    actual = test_y[i:i+1].reshape(1,4)
